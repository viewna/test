	...
	//--------------------- html5  지원되는 브라우저에서 사용하는 함수  --------------------------
 	...
 	function updateViewTotalSize(){
 		var nViewTotalSize = Number(parseInt((nTotalSize || 0), 5) / (1024*1024));
 	...
 	function setUnitString (nByte) {
 		...
 		if( nByte < 1024) {
 			...
 			nImageSize = Number(parseInt((nByte || 0), 5) / (1024*1024));
 			sUnit = 'MB';
 			return nImageSize.toFixed(2) + sUnit;
 		} else {
 			nImageSize = Number(parseInt((nByte || 0), 5) / 1024);
 			sUnit = 'KB';
